...
setup_vm_image() {
    ...
    if [[ -f "$IMG_FILE" ]]; then
        ...
        if wget --progress=bar:force "$IMG_URL" -O "$IMG_FILE.tmp" 2>/dev/null; then
            ...
            print_status "INFO" "You can manually download the image and place it at: $IMG_FILE"
            exit 1
        fi
    fi
    
    # Resize the disk image if needed
    print_status "INFO" "Creating/resizing disk image to $DISK_SIZE..."
    if ! qemu-img create -f qcow2 -F qcow2 -b "$IMG_FILE" "$IMG_FILE.tmp" "$DISK_SIZE" 2>/dev/null; then
    ...
    if load_vm_config "$vm_name"; then
        ...
        if is_vm_running "$vm_name"; then
            ...
            if [[ -n "$qemu_pid" ]]; then
                ...
                echo "║                                                                  ║"
                echo "║   Process Stats:                                                 "
                ps -p "$qemu_pid" -o pid,%cpu,%mem,sz,rss,vsz --no-headers | while read pid cpu mem sz rss vsz; do
                    echo "║     PID: $pid | CPU: $cpu% | MEM: $mem% | Size: $sz           "
                done
            fi
        else
            echo "║   Status:        🔴 Stopped                                       "
